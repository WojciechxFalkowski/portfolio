<template>
  <ul>
    <li v-for="link in navigation" :key="link.label" class="relative">
      <NuxtLink
        :to="{ hash: link.to }"
        :class="[
...
onMounted(() => {
  ...
  observer = new IntersectionObserver(
    (entries) => {
      ...
      // Jeśli maxRatio = 0.3, znaczy że żadna sekcja nie jest w widoku
      activeSection.value = maxRatio > 0.3 ? bestSectionId : "";
    },
    {
